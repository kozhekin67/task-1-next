...
const Header = ({ className }) => {
  return (
    <header
      id="header"
      className={cx(s.tittlen, className)}
    >
      <div className={s.headerBlock}>
        <nav className={s.headerBlock__menu}>
        ...
        <div className={cx(s.headerBlock__title, s.title)}>
          <h1 className={cx(s.title__text)}>
            Идеальные путешествия существуют
          </h1>
        </div>
        <div className={cx(s.headerBlock__introduction, s.introduction)}>
          <p>
            Идейные соображения высшего порядка, а также рамки и место обучения
            кадров
          </p>
        </div>
        <form className={cx(s.headerBlock__findtour, s.findtour)}>
          <Button
            className={cx(s.findtour__searchButton)}
            text="Найти тур"
          />
        </form>
      </div>
